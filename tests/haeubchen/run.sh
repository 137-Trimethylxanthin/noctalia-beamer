...
# parsing and command-building half of each compositor backend (compositor JSON in, neutral model
...
(cd "$OUT" && uv run --quiet "$HERE/../fiaker/transpile.py" \
  ...
  "$HERE"/../../haeubchen/lib/backend/{stream,hyprland,sway,scroll,niri,mango}.luau >/dev/null)
