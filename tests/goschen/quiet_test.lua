...
check("a generic engine name is never muted", quiet.restoreName(stream(1, "discord", "Chromium")) == nil)
check("... nor WebRTC's", quiet.restoreName(stream(1, "discord", "WEBRTC VoiceEngine")) == nil)
check("... nor an ALSA plug-in", quiet.restoreName(stream(1, "x", "ALSA plug-in [discord]")) == nil)
check("a generic name with a distinct id is fine", quiet.restoreName(stream(1, "x", "Chromium", false, { ["application.id"] = "com.discordapp.Discord" }))
  == "Chromium|com.discordapp.Discord")
