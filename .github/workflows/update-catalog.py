...
"""Regenerate catalog.toml from the plugin subdirectory's plugin.toml.

Vendored from noctalia-dev/official-plugins (.github/workflows/update-catalog.py),
with one change: the generated header no longer says "do not include it in your
commit". A self-hosted git plugin source is read through the committed catalog, so
here it has to be in the tree.

Run it by hand after a version bump, or let update-catalog.yml do it on push.
"""
...
def render_catalog(plugins: list[dict]) -> str:
    lines = [
        "# This file is auto-generated by .github/workflows/update-catalog.py.",
        "# Do not edit manually; edit the plugin's plugin.toml and let CI regenerate it.",
        "# Unlike in noctalia-dev/official-plugins, this file IS committed: a git plugin",
        "# source is read through the catalog on the branch users fetch.",
